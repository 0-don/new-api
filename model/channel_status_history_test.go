...
// ChannelFlapStats must aggregate per-channel transition counts and attribute
...
func TestChannelFlapStats(t *testing.T) {
	require.NoError(t, DB.AutoMigrate(&ChannelStatusHistory{}))
	t.Cleanup(func() { DB.Exec("DELETE FROM channel_status_histories") })
	DB.Exec("DELETE FROM channel_status_histories")
	...
	stats, err := ChannelFlapStats(0, "transitions", 0)
	require.NoError(t, err)
	require.Len(t, stats, 2)

	byId := map[int]*ChannelFlapStatRow{}
	for _, s := range stats {
		byId[s.ChannelId] = s
	}
	...
	statsByUptime, err := ChannelFlapStats(0, "uptime", 0)
	require.NoError(t, err)
	assert.Equal(t, 1, statsByUptime[0].ChannelId)

	// limit caps the result set.
	limited, err := ChannelFlapStats(0, "transitions", 1)
	require.NoError(t, err)
	require.Len(t, limited, 1)
	assert.Equal(t, 1, limited[0].ChannelId)
}

// GetChannelStatusHistory must filter and paginate newest-first.
func TestGetChannelStatusHistoryFilter(t *testing.T) {
	require.NoError(t, DB.AutoMigrate(&ChannelStatusHistory{}))
	t.Cleanup(func() { DB.Exec("DELETE FROM channel_status_histories") })
	DB.Exec("DELETE FROM channel_status_histories")

	seed := []*ChannelStatusHistory{
	...
	// filter by channel 7
	rows, total, err := GetChannelStatusHistory(ChannelStatusHistoryFilter{ChannelId: 7}, 0, 10)
	require.NoError(t, err)
	...
	rows, total, err = GetChannelStatusHistory(ChannelStatusHistoryFilter{StatusCode: 400}, 0, 10)
	require.NoError(t, err)
